...
<section class="restaurantes">
	...
	<c:forEach items="${RESTAURANTES}" var="REST">
		<div class="d-flex justify-content-center align-items-center">
			<div
				class="card mb-3 restaurante shadow <c:if test=" ${REST.disponible==false} ">no-disponible</c:if>">
				<div class="row no-gutters">
					<div class="col-md-4 text-center my-auto">
						<c:if test="${not empty REST.imageName}">
							<img src="img/restaurantes/${REST.imageName}"
								class="card-img img-restaurante img-fluid">
						</c:if>
						<c:if test="${empty REST.imageName}">
							<img src="img/restaurantes/defaultRestaurante.jpg"
								class="card-img">
						</c:if>
					</div>
					<div class="col-md-8">
						<div class="card-body">
							<h2 class="card-title bebas">${REST.nombre}
								<a href="restaurante/menu?id=${REST.idRestaurante}"
									class="float-right btn btn-dark">Ver Men&uacute</a> <a
									href="pedidosPorRestaurante?id=${REST.idRestaurante}"
							...
							<p class="card-text">
								...
								<c:if test="${not empty REST.urlMaps}">
									<a href="${REST.urlMaps}" target="_blank" class="ml-1"> <img
										class="location-icon rounded"
										src="img/restaurantes/Localizacion.png">
									</a>
								</c:if>
							</p>
							<p class="card-text">
